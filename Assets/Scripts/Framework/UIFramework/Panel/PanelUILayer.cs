...
namespace KidGame.UI
{
    ...
    {
        [SerializeField] [Tooltip("优先级并行层的设置。注册到此层的面板将根据其优先级重新归属到不同的并行层对象.")]
        ...
        public IPanelController GetRegisteredScreen(string screenId)
        {
            if (registeredScreens.TryGetValue(screenId, out var ctl))
            {
                return ctl;
            }
